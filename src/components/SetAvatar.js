              ...
              {useDefaultAvatars
                ...
                : avatars.map((base64String, i) => (
                    <div
                      ...
                      onClick={() => handleAvatarSelect(i)}
                    >
                      {/* Display API SVG using data URL */}
                      <img
...
//         const avatarArray = [];
//         const maxChoice = 1000;
//         // setLoading(true); // Set loading state if you add one

//         // Array to hold promises
//         const promises = [];

//         for (let i = 0; i < 4; i++) {
//             const randomSeed = `${Date.now()}${Math.round(Math.random() * maxChoice)}`; // Ensure unique seed
//             // Call YOUR backend proxy endpoint
//             const proxyUrl = `${hostUrl}/api/utils/get-avatar/${randomSeed}`; // Adjust path if needed
//             promises.push(axios.get(proxyUrl, { responseType: 'arraybuffer' })); // Request as arraybuffer
//         }
...
//                     avatarArray.push(buffer.toString("base64"));
//                 } else {
//                     // Log error if a specific request failed
//                     console.error("Failed to fetch one avatar:", result.reason || 'Unknown error');
//                     // Optionally push a placeholder or handle differently
//                 }
//             });

//             if (avatarArray.length === 0 && results.some(r => r.status === 'rejected')) {
//                 // If ALL requests failed, show a general error
//                 toast.error("Error fetching avatars. Please try later!", toastStyle);
//                 // Consider NOT navigating away automatically here, let the user decide.
//                 // navigate("/");
//             }
...
//         } catch (error) {
//             // Catch errors related to Promise.allSettled itself (unlikely)
//             console.error("General error fetching avatars:", error);
//             toast.error("Error fetching avatars. Please try later!", toastStyle);
//             // Consider NOT navigating away automatically here.
//             // navigate("/");
//         } finally {
//              // setLoading(false); // Set loading state back
//         }
//     };
//     getimg();
