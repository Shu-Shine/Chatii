...
export default function SearchResults(props) {
  ...
  // if (!props.searchTerm) {  // if 不能在useMemo前使用
  //   navigate("/"); // 重定向到主页
  //   return;
  // }
  ...
  const filteredMessagesResults = useMemo(() => {
    ...
    return props.history.filter((message) =>  // array of messages
  ...
  return (
    <Container>
       ...
       <div className="search-header">
         <button className="back-button" onClick={() => props.setIsSearching(false)}>
           ← Back
         </button>
         <h2>Results for: <span>"{props.searchTerm}"</span></h2>
      </div>
      ...
      <div className="results-content">
        ...
        {hasMessageResults && (
            ...
            <div className="results-list">
                ...
                <div
                    ...
                    onClick={() => props.onMessageResultClick(message.content, message.users, message.sender)}
                >
                    {/* <div className="avatar">
        ...
        {!hasContactResults && !hasMessageResults && (
             <p className="no-results overall-no-results">No contacts or messages found for "{props.searchTerm}".</p>
        )}
...
const Container = styled.div`
  ...
  .search-header {
      ...
      h2 { margin: 0 1rem; font-weight: normal; font-size: 1.2rem; text-align: center; flex-grow: 1; span { font-style: italic; color: chocolate; } }
      .back-button { background-color: chocolate; color: white; border: none; padding: 0.5rem 1rem; border-radius: 0.3rem; cursor: pointer; font-size: 0.9rem; &:hover { background-color: #d2281e; } }
  }
  ...
  .results-content {
      ...
      flex-direction: column;
      gap: 1.5rem;
      &::-webkit-scrollbar { width: 0.4rem; &-thumb { background-color: #ffffff39; border-radius: 1rem; } }
  }

  .results-section {
    h3 { color: #ffffffb0; border-bottom: 1px solid #ffffff30; padding-bottom: 0.5rem; margin-top: 0; margin-bottom: 1rem; font-size: 1rem; font-weight: bold; }
  }

  .results-list { display: flex; flex-direction: column; gap: 0.8rem; }

  /* Shared item base style */
  .contact-result-item, .message-result-item {
    display: flex;
    ...
    &:hover { background-color: #ffffff34; }
  }

  /* Contact specific */
  .contact-result-item {
    .avatar img { height: 3rem; width: 3rem; border-radius: 50%; object-fit: cover; }
    .username span { font-size: 1.1rem; font-weight: 500; }
  }
  ...
  .no-results { text-align: left; margin-top: 0.5rem; color: #ffffff90; font-style: italic; font-size: 0.9rem; }
  .overall-no-results { text-align: center; margin-top: 2rem; font-style: normal; font-size: 1rem; color: #ffffffb0; }
`;
