...
export default function ChatEntry(props) {
  ...
  const handleSendMsg = async (content) => {   
    ...
    if(props.currentUser && props.activeChat.recipient){
      ...
      props.setActiveChat({ recipient:props.activeChat.recipient, messages: msgs })
  ...
  return (
    <StyleContainer>
      <div className="emoji-container">
        <div className='emoji' >
          {/* <i className="emoji-icon fa-regular fa-face-grin-wide" onClick={handleClick}></i>
          {showEmojiPicker && (<EmojiPicker onEmojiClick={handleEmoji}/>) }   */}
        </div>
      </div>

      <form className='entry-container' onSubmit={handleSubmit}>
        <input type='text' value={props.content} onChange={(event)=>props.setContent(event.target.value)}/>
        <button type='submit' ><i className="button-icon fa-regular fa-paper-plane"></i></button> 
      </form>
